    ...
    private static double[] calcFeatureCounts(InstanceList ilist) {
        ...
        for (int i = 0; i < ilist.size(); i++) {
            ...
            if (data instanceof FeatureVector) {
                countVector(counts, (FeatureVector) data);
            } else if (data instanceof FeatureVectorSequence) {
                FeatureVectorSequence fvs = (FeatureVectorSequence) data;
                for (int j = 0; j < fvs.size(); j++) {
                    countVector(counts, fvs.get(i));
                }
            } else {
                throw new IllegalArgumentException("Currently only handles FeatureVector and FeatureVectorSequence data");
            }
        }
        return counts;
    }

    private static void countVector(double[] counts, FeatureVector fv) {
